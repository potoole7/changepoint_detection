...
\section{Spanish temperature and drought data}\label{sec:data}

% Old
% We analyse daily maximum temperature and precipitation observations from 40 meteorological stations distributed across mainland Spain over the period 1960--2020. 
% The observations were obtained from the European Climate Assessment Dataset (ECAD), which provides quality-controlled daily meteorological records from stations across Europe \citep{ECADdaily}. 
% The station locations are shown in the left-hand panel of Figure~\ref{fig:app_intro}.
% \textcolor{red}{Confirm and state the record-length and data-availability
% criteria used to select the 40 stations, if applicable.}
% 
% We aggregate the daily observations to a weekly resolution, taking the maximum
% recorded temperature and total accumulated precipitation within each week.
% This aggregation reduces the influence of very short-range temporal variation
% and the proportion of zero precipitation observations, although it does not
% necessarily remove temporal dependence completely.
...
Daily maximum temperature and precipitation data for 40 locations across Spain for 1960-2020 were obtained from the European Climate Assessment Dataset (ECAD) \citep{ECADdaily}. Figure~\ref{fig:app_intro} left panel shows that the sites considered cover all areas of Spain. We aggregate to a weekly resolution to reduce temporal dependence, taking the maximum recorded temperature and total accumulated precipitation within each week. 

% Our interest is in the extremal dependence between high temperatures and
% prolonged dry conditions. Since land degradation is typically associated with
% sustained precipitation deficits rather than precipitation during a single
% week, we use the Standardised Precipitation Index (SPI) of \citet{McKee1993}
% rather than weekly precipitation totals directly. At each station, the SPI for
% a given week is calculated from precipitation accumulated over a rolling
% 13-week window comprising that week and the preceding 12 weeks. The accumulated
% precipitation is modelled using a Gamma distribution whose parameters are
% estimated using unbiased probability-weighted moments, and the fitted
% cumulative probability is transformed to the standard Gaussian scale.

Since land degradation is typically associated with prolonged drought rather than short-term precipitation deficits, we consider the Standardised Precipitation Index (SPI) of \cite{McKee1993} instead of the weekly precipitation totals. SPI measures deviations on accumulated precipitation from its long-term climatological distribution and therefore provides a more informative summary of drought conditions. Moreover, temperature dynamics may respond more strongly to drought conditions than the precipitation recorded during the current week. We calculate the SPI for a week and site based on the precipitation accumulated over a 13-week window which includes the current week. 
...
\item Only stations satisfying the required record-length and data-availability criteria are retained. \textcolor{red}{State criteria for choosing stations/locations (description in code)}. \textcolor{orange}{CR: Is this necessary? The map shows 40 locations. Are we really removing any of them? I don't want to look through your code}
...
% Under the usual SPI convention, negative values represent drier-than-normal
% conditions. We therefore define the drought index $D=-\mathrm{SPI}$, so that
% larger values correspond to more severe drought conditions. Our analysis
% focuses on the dependence between extremely high weekly maximum temperatures
% and extremely high values of $D$.

Since, under the usual SPI convention, negative values represent drier-than-normal conditions, we define the drought index $D=-\mathrm{SPI}$. Larger values of $D$ therefore correspond to more sever drought conditions. In the following we analyse the dependence between extremely high values for $D$ and the weekly maximum temperature. 

% The right-hand panels of Figure~\ref{fig:app_intro} suggest that the relationship
% between weekly maximum temperature and the drought index is weaker at Daroca,
% in Aragon, than at Jerez de la Frontera, in Andalusia. This is broadly
% consistent with regional climatological differences: Andalusia typically has
% a pronounced summer dry season, whereas precipitation in Aragon is generally
% lower but more evenly distributed throughout the year. Drought conditions in
% Andalusia may therefore be more closely associated with episodes of extreme
% heat. The panels also indicate seasonal differences in the dependence
% structure, motivating separate dependence and changepoint analyses for winter,
% spring, summer and autumn.

Figure \ref{fig:app_intro} suggests that the relationship between weekly maximum temperature and the drought index is weaker for Daroca, Aragon, than for Jerez de la Frontera in Andalusia. These differences are broadly consistent with regional climatological patterns. Andalusia typically experiences a pronounced dry season during summer, with little precipitation over extended periods, whereas Aragon generally receives lower but more evenly distributed precipitation throughout the year. Consequently, drought conditions in Andalusia may be more closely linked to episodes of extreme heat than in Aragon. We also observe slight seasonal differences in the dependence structure and this motivates applying our changepoint procedure separately for each season. 

We organise the observations into \emph{season-years}, which form the temporal units used throughout the simulation study and Spanish application. 
For spring, summer and autumn, the season-year is the calendar year in which the observations occur. 
Winter spans two calendar years, so December is assigned to the same season-year as the following January and February. 
Each season-year contains 12 weekly observations. 
...
Right: weekly maximum temperature against the 13-week drought index $D=-\mathrm{SPI}$ at the two highlighted stations, with observations coloured by season.
}
\label{fig:app_intro}
\end{figure}
