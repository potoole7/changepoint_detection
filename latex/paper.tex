...
Given the data for the time blocks $\mathcal{T}_1(\tau)$ and $\mathcal{T}_2(\tau)$, we obtain estimates $\hat{M}_1(\tau)$ and $\hat{M}^{(2)}(\tau)$ of $M^{(1)}(\tau)$ and $M^{(2)}(\tau)$, respectively, by first estimating the CE model parameters via maximum likelihood inference, followed by Monte Carlo integration for evaluating \eqref{eq:jsga_integral}. Two statistics are considered to test $H_0$ against $H_1$. The first is the Frobenius norm of the difference between $\hat{M}^{(1)}(\tau)$ and $\hat{M}^{(2)}(\tau)$: 
\begin{equation} 
\label{eq:frob}
...
\end{equation}
The Frobenius norm~\eqref{eq:frob} captures the overall magnitude of change, whereas the infinity norm~\eqref{eq:infinity} is only sensitive to the largest pairwise difference. Consequently, their relative performance under non-stationarity is expected to depend on whether changes occur across many sites or are concentrated in a small number of locations. For notational brevity, we write $N_{\mathrm{obs}}(\tau)$ in the rest of the section when referring to $N_{\mathrm{obs}}^{(F)}(\tau)$ or $N_{\mathrm{obs}}^{(I)}(\tau)$.
...
\textcolor{red}{Note I've just included lots of figures here for completeness}

\subsubsection{Global Results}\label{subsubsec:sim_global}

\begin{figure}[htb]
\centering
\includegraphics[width=0.9\linewidth]{plots/changepoint_pval_facet_global.png}
\caption{
  GAM-smoothed p-values against season-year for global simulation setting with 50 repetitions and 200 permutations. 
  Lighter lines show individual realisations. 
  5\% and 10\% significance lines shown in grey and green dotted lines. 
}
\label{fig:sim_global_pvals}
\end{figure}

\begin{figure}[htb]
\centering
\includegraphics[width=0.9\linewidth]{plots/changepoint_bar_global.png}
\caption{
  Barplot showing number of detected changepoints for each season-year and norm type, at the 5\% and 10\% significance levels.
}
\label{fig:sim_global_bar}
\end{figure}
...
\includegraphics[width=0.9\linewidth]{plots/changepoint_bar_global.png}
\caption{
  Barplot showing percentage of repetitions for each season-year and norm type, that are labelled a changepoint, at the 5\% and 10\% significance levels.
}
\label{fig:sim_global_bar_perc}
\end{figure}

\subsubsection{Local Results}\label{subsubsec:sim_local}

\begin{figure}[htb]
\centering
\includegraphics[width=0.9\linewidth]{plots/changepoint_pval_facet_local.png}
\caption{
  GAM-smoothed p-values against season-year for local simulation setting with 100 repetitions and 200 permutations. 
  Lighter lines show individual realisations. 
  5\% and 10\% significance lines shown in grey and green dotted lines. 
}
\label{fig:sim_local_pvals}
\end{figure}

\begin{figure}[htb]
\centering
\includegraphics[width=0.9\linewidth]{plots/changepoint_bar_local.png}
\caption{
  Barplot showing number of detected changepoints for each season-year and norm type, at the 5\% and 10\% significance levels.
}
\label{fig:sim_local_bar}
\end{figure}
