...
\item The seasonal separation in drought-index values is less pronounced at Daroca, whereas Jerez de la Frontera exhibits clearer seasonal variation in both temperature and drought conditions. % \todo{Mention explicit figures for means/95th quantiles??}
...
\item In particular, the resulting clustering solutions exhibited clear spatial contiguity. % \todo{Add a sensitivity analysis for the dependence threshold.}
...
\item The conditional fits from the two conditioning directions are combined into the aggregated dissimilarity matrix defined in Section~\ref{sec:method}. % \todo{Test for others}
\item All screening, testing and clustering calculations reported below use this matrix. % \todo{Confirm terminology and add sensitivity results for the alternative dissimilarity matrices, if required.}
\end{itemize}
...
\item This was the shortest common block length that produced stable fits across all seasons while retaining a useful range of candidate changepoints. % \todo{Present the alternative block lengths as a sensitivity analysis or in the supplementary material(?).}
\end{itemize}
...
\item Across the four seasons, the largest Frobenius norm screening value occurs for summer around 1988. % \todo{Cite exact value}
...
\item These divisions treat a candidate (t) as a change between season-years $(t)$ and $(t+1)$. \todo{Confirm or update the selected divisions after completing the multiplicity-adjusted changepoint analysis.}
\end{itemize}
