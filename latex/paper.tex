...
Y_{i,s}^{(t)} =
\begin{cases}
\log\left\{2 F_{i,s}^{(t)}(X_{i,s}^{(t)})\right\} & \mbox{for}~F_{i,s}^{(t)}\left(X_{i,s}^{(t)}\right) < 0.5,\\
-\log\left\{2 \left[1-F_{i,s}^{(t)}(X_{i,s}^{(t)})\right]\right\} & \mbox{for}~F_{i,s}^{(t)}\left(X_{i,s}^{(t)}\right) \geq 0.5,
\end{cases}
\label{eq:Laplace}
\end{equation}
...
We fit this model separately for each conditioning variable and spatial location, without imposing spatial smoothness. The threshold $u_{i,s}^{(t)}$ is fixed at a high quantile $u$ of the standard Laplace distribution, for all $i$ and $s$, to ensure that the resulting CE models are comparable across locations.
...
\subsection{A divergence measure for condtional extremes models}\label{subsec:div}
...
\cite{OToole2026clustering} introduce the conditional extremal skew-geometric Jensen--Shannon divergence as a dissimilarity measure to quantify the pairwise similarity of CE models; the measure itself is based on the skew-geometric Jensen--Shannon divergence \citet{Nielsen2019}. Let $H_{i,s}^{(t)}$ denote the distribution function of the CE model in \eqref{eq:ce_conditional_gaussian} for $\boldsymbol{Y}_{-i,s}^{(t)}\mid (Y_{i,s}^{(t)} = y)$. The skew-geometric Jensen--Shannon divergence of $H_{i,s}^{(t)}$ and $H_{i,s'}^{(t)}$ is then formally defined as
\begin{equation}
\label{eq:jsga}
\JSGa{}\left(H_{i,s}^{(t)} \Vert H_{i,s'}^{(t)}\right)
=
\lambda
\KL\left\{
G_{\lambda}\left(H_{i,s}^{(t)},H_{i,s'}^{(t)}\right) \Vert H_{i,s}^{(t)}
\right\}
+
(1-\lambda)
\KL\left\{
...
\right\},
\end{equation}
where $\KL$ denotes the Kullback--Leibler divergence, $G_{\lambda}\left(H_{i,s}^{(t)},H_{i,s'}^{(t)}\right)$ is the distribution with density proportional to the weighted geometric mean of the densities corresponding to $H_{i,s}^{(t)}$ and $H_{i,s'}^{(t)}$, and $\lambda\in[0,1]$. We set $\lambda=0.5$, yielding a symmetric, non-negative measure of dissimilarity. Due to the Gaussian assumption~\eqref{eq:ce_conditional_gaussian}, the expression in~\eqref{eq:jsga} can be evaluated in closed form; see \citet{Nielsen2019} and \citet{OToole2026clustering} for details.
...
\begin{itemize}
\item Cluster membership is associated with broad geographical structure, although geography does not completely determine the solutions.
\item Autumn exhibits a particularly clear longitudinal ordering, while Summer shows a comparatively strong latitudinal division.
\item Winter has a more complicated geographical structure and changes substantially after 1998.
\item Elevation overlaps considerably between clusters and does not provide a consistent explanation of membership across seasons.
\item Distance to the coast provides clearer separation, particularly in Winter and Summer.
\item Pre-changepoint Winter cluster 2 and Summer cluster 2 generally contain more coastal locations, whereas other clusters contain more inland locations.
\item High temperature quantiles clearly distinguish some clusters, including the warmer cluster 2 in pre-changepoint Winter and in Summer.
\item Some temperature-quantile separation weakens after the changepoints, particularly in Winter and Autumn.
\item High DPI quantiles provide some of the clearest marginal distinctions between clusters.
\item Winter cluster 3 has the highest DPI quantiles, post-changepoint Summer cluster 2 has the highest DPI quantiles, and pre-changepoint Autumn shows a progression from lower values in cluster 1 to higher values in cluster 3.
\item The empirical $\widehat{\chi}(0.9)$ and $\widehat{\overline{\chi}}(0.9)$ estimates also distinguish several clusters, confirming that the solutions represent different extremal-dependence regimes rather than only different marginal climates.
\item Pre-changepoint Winter cluster 2 has relatively strong tail dependence, whereas cluster 3 has relatively weak tail dependence.
\item In Autumn, cluster 2 generally has weaker tail dependence, while cluster 3 has especially strong tail dependence after 1986.
\item Summer shows greater overlap in $\widehat{\chi}(0.9)$, although some separation remains in $\widehat{\overline{\chi}}(0.9)$.
\item The physical interpretation of a cluster number can change between seasons and periods, so cluster labels should not be treated as having a universal meaning.
\item The distance-to-coast axis currently appears to contain values in metres despite being labelled in kilometres and should be corrected before presentation.
\end{itemize}
