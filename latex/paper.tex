...
% Intro to the data and the application
\begin{itemize}
  \item We analyse daily maximum temperature and precipitation observations from 40
meteorological stations distributed across mainland Spain over the period 1960--2020. 
  ...
  \item The station locations are shown in the left-hand panel of Figure~\ref{fig:app_intro}. 
  \item Daily maximum temperature and precipitation totals are obtained from the European Climate Assessment Dataset (ECAD) which provides quality-controlled daily meteorological records from stations across Europe \citep{ECADdaily}. 
  \item Daily observations are aggregated to weekly resolution. 
...
% SPI description
\begin{itemize}
  \item Our interest is in the extremal dependence between high temperatures and dry conditions. 
  ...
  \item At each station, SPI is calculated from precipitation accumulated over a rolling 13-week window, so that the value assigned to a week summarises precipitation during that week and the preceding 12 weeks. 
  \item The accumulated precipitation is modelled using a Gamma distribution whose parameters are estimated using unbiased probability-weighted moments, and its fitted cumulative probability is transformed to the standard Gaussian scale.
  ...
  $D=-\mathrm{SPI}$,
  so that larger values correspond to drier conditions and positive association with high temperature has a natural interpretation.
\end{itemize}

% Brie motivation of seasonal analysis
\begin{itemize}
  ...
  \item Both variables exhibit spatial and seasonal heterogeneity. 
  \item In particular, weekly maximum temperature differs substantially between seasons at both stations, while the seasonal separation in drought-index values is more pronounced at Jerez de la Frontera.
  ...
  \item This allows both the marginal behaviour and the extremal dependence structure to vary seasonally, and avoids the need to model seasonal variation explicitly.
\end{itemize}

% season-year setup
\begin{itemize}
  \item We organise the observations into \emph{season-years}, which form the temporal units used throughout the simulation study and the Spanish application. 
  ...
  \item Each season-year contains 12 weekly observations. 
  \item A candidate season-year $t$ represents a possible change between season-years $t$ and $t+1$, i.e.\ the dependence structure is assumed to be constant before $t$ and after $t+1$, but may differ between these two blocks.
  % \item In a permutation procedure, complete season-years are reassigned as indivisible units, using the same reassignment for both variables and all stations. \textcolor{red}{Will have mentioned permutation procedure in the introduction.}
  % \item This preserves dependence among the weekly observations within a season-year and the contemporaneous spatial correspondence among stations.
...
The simulation design follows the observational structure of the Spanish drought data introduced in Section~\ref{sec:data}, with two variables observed weekly at multiple sites and grouped over a sequence of ``season-years'' (defined as the period from October to September)
