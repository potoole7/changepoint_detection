...
Y_{i,s}^{(t)} =
\begin{cases}
\log\left\{2 \left(F_{i,s}^{(t)}\right)^{-1}(X_{i,s}^{(t)})\right\} & \mbox{for}~X_{i,s}^{(t)} < F_{i,s}^{(t)}(0.5),\\
-\log\left\{2 \left[1-\left(F_{i,s}^{(t)}\right)^{-1}(X_{i,s}^{(t)})\right]\right\} & \mbox{for}~X_{i,s}^{(t)} \geq F_{i,s}^{(t)}(0.5),
\end{cases}
\label{eq:Laplace}
\end{equation}
...
\cite{OToole2026clustering} introduce a dissimilarity measure based on the skew-geometric Jensen--Shannon divergence \citet{Nielsen2019} to quantify the similarity of CE models. 


To compare extremal dependence structures across spatial locations, we use the divergence-based framework of \citet{OToole2026clustering}. 
