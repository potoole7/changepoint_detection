...
A common threshold $u_i$, defined on the standard Laplace scale, is used across locations to ensure that the resulting conditional distributions are comparable. 
...
\,\mathrm{d}y,
\end{equation}
where $h(y\mid Y_i>u_i)$ is the conditional density of a standard Laplace variable above $u_i$. 
...
The same sample is used across locations and temporal blocks, preventing additional Monte Carlo variation from being introduced into comparisons between blocks.
...
\begin{enumerate}[label=\arabic*.]
  ...
  \item \textbf{Refit CE models:} For the permuted dataset, reapply the exceedance threshold $u_i$ for each conditioning variable and refit the CE model in each pseudo-block, yielding divergence matrices  $M^{(b)}(T_1)$ and $M^{(b)}(T_2)$, for $b = 1,\dots,B$ permutations.
