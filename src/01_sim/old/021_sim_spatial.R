...
# TODO Manually convert to Laplace scale, rather than fitting GPDs

# TODO Move functions to functions file

# TODO Perform permutation test (and sliding) using evgam to fit CE
# (on spatial simulated data)
# Hopefully evgam will be less susceptible to block changing
...
gen_t_loc <- \(cor_t, n, n_vars = 2, df_t = 3, laplace_trans = FALSE) {
  # generate data
...
# Use 0.5, as it gives tighter clusters
...
data_spat_block |>
  mutate(
    name = stringr::str_remove(name, "loc_"),
...
ggplot(dist[[2]], which = "image") # can see location 9 is different, as it's closer to cluster 2
...
plot_df |>
  ...
  mutate(
    ...
    method = factor(method, levels = c("CeCl", "evgam")),
    # change parameter to alpha and beta and compile them nicely
    param = recode(param, a = "Alpha", b = "Beta")
  ) |>
  ggplot(aes(x = x_loc, y = y_loc, colour = value)) +
  geom_point(size = 3) +
  # facet_grid(param ~ block + method) +
  facet_grid(param ~ method + block) +
  scale_colour_viridis_c() +
...
# For CeCl, you even get negative alpha values! Definitely don't want that
