...
# TODO - Plot CE diagnostic plots for each location (and variable)
# TODO - Plot clustering solutions for different thresholds (i.e. sensitivity)
#        - Plot for each conditioning variable also!!
...
dep_vars <- c("rain")
k_vals <- 1:5 # TODO Move to metadata
...
# seems like k = 2 for season specific, and
...
twgss_vals_var |>
  ...
  scale_x_continuous(breaks = c(k_vals)) +
  cecl_theme()
# all confidently say 3 clusters as well
...
plot_one_map <- \(clust_sets, dist_type, k, season, var = "rain", title = NULL) {
  p <- plt_clust_map(
    ...
    clust_sets[[dist_type]][[as.character(k)]][[paste(var, season, sep = "_")]]$pam,
    plot_medoids = FALSE,
    pt_size = 6
  ) +
    cecl_theme()
...
make_season_page <- \(season, k, type = c("map", "heatmap"), title = NULL) {
  ...
  plots <- lapply(names(clust_sets), \(dist_type) {
    if (type == "map") {
      plot_one_map(clust_sets, dist_type, k, season, title)
    } else {
      plot_one_heatmap(clust_sets, dist_type, k, season, title)
    }
  })
